...
export const createRecipeTry = async (recipeId, tryData) => {
  try {
    ...
    console.log("[v0] Try data:", {
      ...tryData,
      imageUrl: tryData.imageUrl ? "image provided" : "no image",
    });
...
export const deleteRecipeTry = async (recipeId, tryId) => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/recipes/${recipeId}/tries/${tryId}`,
      {
        method: "DELETE",
        headers: getAuthHeaders(),
      },
...
export const searchByIngredients = async (ingredients) => {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/search?q=${encodeURIComponent(ingredientQuery)}`,
    );
